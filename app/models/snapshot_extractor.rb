class SnapshotExtractor < ApplicationRecord
  ...
  def self.fetch_details
    SnapshotExtractor.connection.select_all('SELECT s0."id", s0."from_date", s0."to_date", s0."interval", s0."schedule", f1."exid", f1."timezone", f1."name", s0."requestor" FROM "snapshot_extractors" AS s0 INNER JOIN LATERAL (SELECT * FROM cameras as cam WHERE cam.id = s0."camera_id") AS f1 ON TRUE WHERE (s0."status" = 0) ORDER BY s0."created_at" DESC LIMIT 1').to_hash
  end
